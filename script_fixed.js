...
document.addEventListener('DOMContentLoaded', () => {
                ...
                json.forEach(row => {
                    ...
                    if (className) {
                        branchMap[branchName].subClasses.push({
                            ...
                            profitMargin: parseFloat(row['Hedef KÃ¢r (%)']) || 0
                        });
                    }
                });
            ...
            branchData.forEach(branch => {
                ...
                branch.subClasses.forEach(cls => {
                    ...
                    excelData.push({
                        ...
                        "1 Adet makine için Genel Üretim Gideri Payı (₺)": cls.quantity > 0 ? (clsGUG / cls.quantity) : 0,
                        "1 Adet Makine Maliyeti (₺)": clsUnitCost,
                        "1 Adet Satış Fiyatı (₺)": cls.salePrice || 0,
                        "1 Adet makine için Brüt KÃ¢r (₺)": (cls.salePrice || 0) - clsUnitCost
                    });
                });
            });
    ...
    function updateChart() {
        ...
        const dataProfit = branchData.map(b => (b.branchTotalSales || 0) - (b.branchNetTotal || 0));
    ...
    function calculateGlobal() {
        ...
        branchData.forEach(branch => {
            ...
            branch.branchNetTotal = branch.branchBaseTotal + branch.branchShare + (branch.laborCost || 0) + branch.branchGUG;
            ...
            branch.subClasses.forEach(cls => {
                ...
                const clsNet = cls.baseTotal + clsShare + clsLaborShare + cls.gug;
                const clsUnitCost = cls.quantity > 0 ? (clsNet / cls.quantity) : 0;
                
                if (cls.lastModified === 'margin') {
                    cls.salePrice = clsUnitCost * (1 + (cls.profitMargin || 0) / 100);
                } else {
                    if (clsUnitCost > 0) {
                        cls.profitMargin = (((cls.salePrice || 0) - clsUnitCost) / clsUnitCost) * 100;
                    } else {
                        cls.profitMargin = 0;
                    }
                }
            ...
            branch.branchTotalProfit = branch.branchTotalSales - branch.branchNetTotal;
        ...
        const globalNetTotal = globalBase + globalLabor + globalExpense + totalGUG;
        const globalNetProfit = globalTotalSales - globalNetTotal;
        ...
        if (elNetProfit) {
            elNetProfit.textContent = formatCurrency(globalNetProfit);
        ...
        if (elNetTotalLegacy) {
            ...
            elNetTotalLegacy.innerHTML = `${formatCurrency(globalNetTotal)} <br><span style="font-size:0.8rem; color:var(--text-secondary); font-weight:normal; opacity:0.8;">(${textNet})</span><br><br><span style="color:var(--text-primary);">Toplam Brüt KÃ¢r:</span> <span style="color: ${globalNetProfit >= 0 ? 'var(--success)' : 'var(--danger)'};">${formatCurrency(globalNetProfit)}</span><br><span style="font-size:0.8rem; color:var(--text-secondary); font-weight:normal; opacity:0.8;">(${profitText})</span>`;
        }
    ...
    function calculateDetail() {
        ...
        const branchNetTotal = detailBase + branchShare + branchLabor + branchGUG; // For progress bars
        ...
        branch.subClasses.forEach((cls, cIndex) => {
            ...
            const clsLaborShare = cls.baseTotal * laborMultiplier;
            const clsGUG = cls.gug || 0;
            const clsNet = cls.baseTotal + clsShare + clsLaborShare + clsGUG;
            const clsUnitCost = cls.quantity > 0 ? (clsNet / cls.quantity) : 0;
            ...
            const clsProfit = (cls.salePrice || 0) - clsUnitCost;
        ...
        detailSharedExpense.innerHTML = `${formatCurrency(branchShare)} <br><span style="font-size:0.8rem; color:var(--text-secondary); font-weight:normal; opacity:0.8; letter-spacing:0px;">(${textBranchShare})</span>`;
        
        const textBranchLabor = numberToTurkishText(branchLabor);
        document.getElementById('detailLaborExpense').innerHTML = `${formatCurrency(branchLabor)} <br><span style="font-size:0.8rem; color:var(--text-secondary); font-weight:normal; opacity:0.8; letter-spacing:0px;">(${textBranchLabor})</span>`;
        
        const textDetailBase = numberToTurkishText(detailBase);
        detailTotalBaseUnits.innerHTML = `${formatCurrency(detailBase)} <br><span style="font-size:0.8rem; color:var(--text-secondary); font-weight:normal; opacity:0.8; letter-spacing:0px;">(${textDetailBase})</span>`;
    ...
    function renderMainView() {
        ...
        branchData.forEach((branch, bIndex) => {
            ...
            const netTotal = branch.branchBaseTotal + branch.branchShare + (branch.laborCost || 0) + (branch.branchGUG || 0);
    ...
    function renderDetailView() {
        ...
        branch.subClasses.forEach((cls, cIndex) => {
            ...
            card.innerHTML = `
                ...
                <div class="sm-inputs-row" style="display: grid; grid-template-columns: 1fr 1.2fr 1.2fr 1fr; gap: 1rem;">
                ...
                <div class="branch-results">
                    ...
                    <div class="result-row unit">
                        <span>1 Adet makine için İşçilik Payı:</span>
                        <span id="cls-unit-labor-share-${cIndex}">0 ₺</span>
                    </div>
                    <div class="result-row unit">
                        <span>1 Adet makine için Genel Üretim Gideri Payı:</span>
                    ...
                    <div class="result-row unit" style="margin-top: 1rem; font-size: 1.2rem;">
                        <span>1 Adet makine için Brüt KÃ¢r:</span>
    ...
    if(rateEURInput) rateEURInput.addEventListener('input', () => {
        saveData();
        calculateGlobal();
        if (currentBranchIndex !== null) calculateDetail();
    });
    ...
    function renderCompareResults(scenarioKey, container) {
        ...
        const netCost = globalBase + globalLabor + gExp + gGug;
        const grossProfit = totalSales - netCost;
        
        container.innerHTML = `
            <div class="result-row total"><span class="label">Net Maliyet</span><span class="value">${formatCurrency(netCost)}</span></div>
            <div class="result-row total"><span class="label">Brüt KÃ¢r</span><span class="value" style="color: ${grossProfit >= 0 ? 'var(--success)' : 'var(--danger)'};">${formatCurrency(grossProfit)}</span></div>
