...
start_idx = content.find("const netKarText = (netKar < 0 ? \"Eksi \" : \"\") + numberToTurkishText(Math.abs(netKar));")
if start_idx != -1:
    end_idx = content.find("        `;\n        \n        document.getElementById('factorySummaryResults').innerHTML = html;", start_idx)
    
    if end_idx != -1:
        old_block = content[start_idx:end_idx]
        
        new_block = """const netKarText = (netKar < 0 ? "Eksi " : "") + numberToTurkishText(Math.abs(netKar));
        ...
        const html = `
            ...
            <div class="result-row accordion-header" onclick="this.classList.toggle('open'); this.nextElementSibling.classList.toggle('active')" style="cursor: pointer; margin-top: 1rem;">
...
"""
        ...
        print("Successfully updated renderFactorySummary!")
    else:
