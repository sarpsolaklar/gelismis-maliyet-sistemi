...
pattern = r"const html = `\s*<div class=\"result-row total\" style=\"background: linear-gradient\(90deg, rgba\(236, 72, 153, 0\.1\) 0%, transparent 100%\); border-left-color: var\(--accent-2\);\">\s*<span>Fabrika Net Maliyeti:</span>\s*<span>\$\{formatCurrency\(netMaliyet\)\}</span>\s*</div>\s*<div class=\"result-row total accordion-header open\" onclick=\"this.classList.toggle\('open'\); this.nextElementSibling.classList.toggle\('active'\)\" style=\"margin-top: 1rem; border-left-color: #8b5cf6; background: linear-gradient\(90deg, rgba\(139, 92, 246, 0\.1\) 0%, transparent 100%\);\">\s*<span style=\"display: flex; align-items: center; gap: 8px;\">Fabrika Net Kâr: <span class=\"chevron\">.*?</span></span>\s*<span style=\"color: \$\{netKar >= 0 \? 'var\(--success\)' : 'var\(--danger\)'\};\}>\$\{formatWithPercent\(netKar, netMaliyet\)\}</span>\s*</div>\s*<div class=\"accordion-content active\">\s*<div class=\"result-row total\" style=\"margin-top: 0\.5rem; border-left-color: var\(--accent-1\); background: linear-gradient\(90deg, rgba\(56, 189, 248, 0\.1\) 0%, transparent 100%\);\">\s*<span>Fabrika Faaliyet Kârı:</span>\s*<span style=\"color: \$\{faaliyetKari >= 0 \? 'var\(--success\)' : 'var\(--danger\)'\};\}>\$\{formatWithPercent\(faaliyetKari, netMaliyet\)\}</span>\s*</div>\s*<div class=\"result-row total\" style=\"margin-top: 0\.5rem; border-left-color: var\(--success\); background: linear-gradient\(90deg, rgba\(16, 185, 129, 0\.1\) 0%, transparent 100%\);\">\s*<span>Fabrika Brüt Kâr:</span>\s*<span style=\"color: \$\{brutKar >= 0 \? 'var\(--success\)' : 'var\(--danger\)'\};\}>\$\{formatWithPercent\(brutKar, netMaliyet\)\}</span>\s*</div>\s*</div>"
